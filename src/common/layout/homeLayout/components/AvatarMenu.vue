    ...
    <a-dropdown>
      <div class="trigger">
        <a-row type="flex" justify="end">
          ...
          <a-col :xs="0" :sm="0" :md="0" :lg="0" :xl="0" :xxl="17">欢迎您，陈梓豪</a-col>
        </a-row>
      </div>
      <a-menu slot="overlay" @click="handleMenuClick">
        <a-menu-item key="onChange">
          <a-icon type="tool" />系统设置
        </a-menu-item>
        <a-menu-item key="2">
    ...
    <a-drawer placement="right" :closable="false" :visible="visible" @close="onClose">
      ...
      <a-row>
        <a-col :span="8" style="cursor: pointer;">
          <a-tooltip placement="top">
            <img
              src="@/../static/img/sider.svg"
              width="50"
              height="50"
              @click="setMenuModel('inline')"
            />
            <a-icon type="check" :class="checkClass" v-if="menuModel === 'inline'" />
            <template slot="title">侧边导航栏</template>
          </a-tooltip>
        </a-col>
        <a-col :span="8" style="cursor: pointer;">
          <a-tooltip placement="top">
